...
if [ -f .env.local ]; then
  ...
  [ -z "$FIREBASE_DATABASE_URL" ] && FIREBASE_DATABASE_URL=$(get_env_val "NEXT_PUBLIC_FIREBASE_DATABASE_URL")
...
if [ -z "$FIREBASE_DATABASE_URL" ]; then
  echo "❌ Error: FIREBASE_DATABASE_URL environment variable is not set"
  echo "   Set it with: export FIREBASE_DATABASE_URL='your-database-url'"
  exit 1
fi
