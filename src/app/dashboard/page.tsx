...
export default function DashboardPage() {
  ...
  // Load available roles from questions
  useEffect(() => {
    const loadRoles = async () => {
      try {
        const questions = await fetchQuestions();
        const roles = getAvailableRoles(questions);
  ...
  // Sort filtered interviews by most recent first
  ...
  // Get unique roles from interviews for display (separate from filter dropdown)
  const uniqueInterviewRoles = useMemo(() => {
    const interviewRoles = new Set(allInterviews.map((i) => i.role));
    return Array.from(interviewRoles).sort();
  }, [allInterviews]);

  // Use all available roles for the filter dropdown (73 roles)
  const filterRoles = useMemo(() => {
    // Always use availableRoles (all 73 roles) for the dropdown
    return availableRoles.length > 0
      ? availableRoles.sort()
      : uniqueInterviewRoles;
  }, [availableRoles, uniqueInterviewRoles]);
  ...
  const difficultyDistributionData = useMemo(() => {
    ...
    filteredInterviews.forEach((interview) => {
      difficultyCounts[interview.difficulty] =
        (difficultyCounts[interview.difficulty] || 0) + 1;
    });
  ...
  const handleVerifyQuestions = async () => {
    ...
    try {
      ...
      if (count >= 4000) {
        setQuestionStatus(
          `✅ ${count.toLocaleString()} questions found in Firestore. Comprehensive database loaded!`
        );
      } else if (count >= 595) {
        setQuestionStatus(
          `⚠️ ${count} questions found. Consider importing comprehensive questions (4,000+) for full coverage across 73 roles.`
        );
      } else {
        setQuestionStatus(
          `⚠️ Only ${count} questions found. Expected 4,000+ for full coverage. Re-import using ImportQuestionsButton.`
        );
      }
    } catch (err: any) {
      setQuestionStatus(
        err?.message ||
          "Failed to verify questions. Check your Firebase rules/connection."
      );
    } finally {
        ...
        <div className="max-w-7xl mx-auto space-y-8 p-6">
          ...
          <section className="bg-white border border-gray-200 rounded-xl p-6 shadow-sm">
            <div className="flex items-center justify-between mb-3">
              ...
              >
                {checkingQuestions ? "Checking..." : "Verify Questions"}
              </button>
            </div>
            <p className="text-sm text-gray-600">
              Confirms Firestore has 4,000+ questions across 73 roles. Use this
              after importing.
            </p>
            {questionStatus && <p className="mt-3 text-sm">{questionStatus}</p>}
          ...
          <section className="bg-white border border-red-200 rounded-xl p-6 shadow-sm">
            ...
            <p className="text-sm text-gray-600">
              Delete interviews without multimodal analysis to free up storage
              space. This will permanently delete videos and interview data.
            </p>
          </section>
